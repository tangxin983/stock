	...
	public void buy(String symbol) {
		...
		while (date != END_DATE) {
			if (dates.contains(date)) {
				...
				Double highestHigh = indicators.highestHigh(symbol, date,
						BREAK_DAYS);
				...
				if (stock.getHigh() > highestHigh && volumeRatio >= RATIO) {
					...
					logger.debug(
							...
							stock.getDate(), changeRatio, stock.getHigh(),
							volumeRatio, BREAK_DAYS, highestHigh);
		...
		logger.debug("====={} breakhighBuy summary=====", symbol);
